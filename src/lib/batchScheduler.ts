import { Order, ProductionBatch, ProductionBatchStatus, Product } from '../types';
...
export function getBatchCapacityForDate(
  ...
  return {
    capacity: product.schedule_config?.batch_limit || 10,
    status: 'PLANNED',
  };
}
...
export function getNextAvailableBatch(
  ...
  const { available_days } = product.schedule_config;
  if (!available_days || available_days.length === 0) {
    return null;
  }

  const minLeadDays = product.schedule_config.min_lead_days ?? 1;
  const deliveryWindow = product.schedule_config.delivery_window || '14:00 - 18:00';
...
export function getProductBatchDisplayInfo(
  ...
  if (!product.schedule_config || !product.schedule_config.is_scheduled_only) {
    return {
      ...
      deliveryWindow: '',
      earliestBatch: null,
      nextAvailableBatch: null,
      isFirstBatchFull: false,
      firstBatchNotice: null,
  ...
  const upcoming = getUpcomingBatches(product, orders, 3, new Date(), 1, batches);
