...
DROP POLICY IF EXISTS "Public Read Delivery Items" ON public.order_delivery_items;
CREATE POLICY "Public Read Delivery Items" ON public.order_delivery_items FOR SELECT USING (true);
DROP POLICY IF EXISTS "Public Modify Delivery Items" ON public.order_delivery_items;
CREATE POLICY "Public Modify Delivery Items" ON public.order_delivery_items FOR ALL USING (true) WITH CHECK (true);

-- RPC 1: Reserva Atômica de Capacidade por Fornada (com lock de linha / INSERT ON CONFLICT)
...
DECLARE
    ...
    v_new_status TEXT;
BEGIN
    -- Garante que o registro da fornada exista
    ...
    IF v_batch.status = 'CANCELLED' OR v_batch.status = 'CLOSED' THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', 'Esta fornada foi encerrada ou cancelada pelo estabelecimento.'
        );
    END IF;

    -- Validação estrita de capacidade (não ultrapassar a capacidade)
    IF (v_batch.reserved + p_quantity) > v_batch.capacity THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', format('Capacidade insuficiente na fornada de %s. Vagas disponíveis: %s, solicitado: %s.',
                            p_batch_date, (v_batch.capacity - v_batch.reserved), p_quantity)
        );
    END IF;
...
-- RPC 2: Liberação Atômica de Capacidade por Fornada (em cancelamentos ou rollbacks)
